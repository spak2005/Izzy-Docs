...
async def modify_doc_text(
    ...
    logger.info(
        ...
        f"formatting={any([bold, italic, underline, strikethrough, font_size, font_family, text_color, background_color, link_url])}"
    )
...
# ============================================================================
# PARAGRAPH STYLING TOOLS
# ============================================================================


@server.tool()
@handle_http_errors("apply_paragraph_style", service_type="docs")
@require_google_service("docs", "docs_write")
async def apply_paragraph_style(
    ...
    """
    Applies paragraph-level formatting to a document section.

    Target the paragraph either by:
    - Providing start_index and end_index directly, OR
    - Providing text_to_find to locate text and format its containing paragraph

    Args:
        user_google_email: User's Google email address
        document_id: ID of the document to update
        start_index: Start position of paragraph range (optional if using text_to_find)
        end_index: End position of paragraph range (optional if using text_to_find)
        text_to_find: Text to locate - will format the paragraph containing this text
        match_instance: Which instance of text_to_find to use (1=first, 2=second, etc.)
    ...
    """
    logger.info(
        f"[apply_paragraph_style] Doc={document_id}, start={start_index}, end={end_index}, "
        f"text_to_find={text_to_find}, alignment={alignment}, named_style={named_style_type}"
    )
    ...
    if not any([
        alignment, indent_start is not None, indent_end is not None,
        space_above is not None, space_below is not None, named_style_type,
        keep_with_next is not None, line_spacing is not None
    ]):
        return "Error: At least one paragraph style option must be provided (alignment, indent_start, indent_end, space_above, space_below, named_style_type, keep_with_next, line_spacing)."

    # Determine target range
    if text_to_find:
        # Get document to find text
        doc = await asyncio.to_thread(
            service.documents().get(documentId=document_id).execute
        )

        # Find the text range
        text_range = find_text_range(doc, text_to_find, match_instance)
        if not text_range:
            return f"Error: Could not find instance {match_instance} of text '{text_to_find}' in document."
        ...
        logger.info(f"Found text in paragraph at range {start_index}-{end_index}")

    elif start_index is None or end_index is None:
        return "Error: Must provide either (start_index AND end_index) OR text_to_find."

    # Validate the range
    if end_index <= start_index:
        return f"Error: end_index ({end_index}) must be greater than start_index ({start_index})."

    # Build the paragraph style request
    try:
    ...
    if not request:
        return "Error: No valid paragraph style options were provided."
    ...
    style_details = []
    if alignment:
        style_details.append(f"alignment={alignment}")
    if indent_start is not None:
        style_details.append(f"indent_start={indent_start}pt")
    if indent_end is not None:
        style_details.append(f"indent_end={indent_end}pt")
    if space_above is not None:
        style_details.append(f"space_above={space_above}pt")
    if space_below is not None:
        style_details.append(f"space_below={space_below}pt")
    if named_style_type:
        style_details.append(f"named_style={named_style_type}")
    if keep_with_next is not None:
        style_details.append(f"keep_with_next={keep_with_next}")
    if line_spacing is not None:
        style_details.append(f"line_spacing={line_spacing}")

    link = f"https://docs.google.com/document/d/{document_id}/edit"
    return f"Applied paragraph style ({', '.join(style_details)}) to range {start_index}-{end_index}. Link: {link}"


@server.tool()
@handle_http_errors("format_matching_text", service_type="docs")
@require_google_service("docs", "docs_write")
async def format_matching_text(
    service: Any,
    user_google_email: str,
    document_id: str,
    text_to_find: str,
    ...
    """
    Finds specific text within a Google Document and applies character formatting.

    This tool searches for the specified text and applies formatting to it without
    needing to know the exact document indices.

    Args:
        user_google_email: User's Google email address
        document_id: ID of the document to update
        text_to_find: The exact text string to find and format
        match_instance: Which instance of the text to format (1=first, 2=second, etc.)
        bold: Whether to make text bold (True/False/None to leave unchanged)
        italic: Whether to make text italic (True/False/None to leave unchanged)
        underline: Whether to underline text (True/False/None to leave unchanged)
        strikethrough: Whether to apply strikethrough (True/False/None to leave unchanged)
    ...
    """
    logger.info(
        f"[format_matching_text] Doc={document_id}, text='{text_to_find}', instance={match_instance}"
    )
    ...
    if not text_to_find:
        return "Error: 'text_to_find' is required."

    # Validate that at least one formatting option is provided
    if not any([
        bold is not None, italic is not None, underline is not None,
        strikethrough is not None, font_size, font_family, text_color,
    ...
    logger.info(f"Found text '{text_to_find}' at range {start_index}-{end_index}")
    ...
    if not request:
        return "Error: No valid formatting options were provided."
    ...
    link = f"https://docs.google.com/document/d/{document_id}/edit"
    return f"Applied formatting ({', '.join(format_details)}) to instance {match_instance} of '{text_to_find}'. Link: {link}"
...
@server.tool()
@handle_http_errors("append_to_doc", service_type="docs")
@require_google_service("docs", "docs_write")
async def append_to_doc(
    ...
    """
    Appends text to the very end of a Google Document.

    Args:
        user_google_email: User's Google email address
        document_id: ID of the document to update
        text_to_append: The text to add to the end of the document
        add_newline_before: Whether to add a newline before the appended text (default: True)

    Returns:
        str: Confirmation message with operation details
    """
    logger.info(f"[append_to_doc] Doc={document_id}, text_length={len(text_to_append)}")
    ...
    if not text_to_append:
        return "Error: 'text_to_append' is required."
    ...
    end_index = 1  # Default to beginning if empty

    if body_content:
        last_element = body_content[-1]
        if "endIndex" in last_element:
            # Insert before the final newline (typical doc structure)
            end_index = last_element["endIndex"] - 1

    # Prepare text to insert
    text_to_insert = ("\n" if add_newline_before and end_index > 1 else "") + text_to_append

    # Create and execute the request
    requests = [create_insert_text_request(end_index, text_to_insert)]
    ...
    link = f"https://docs.google.com/document/d/{document_id}/edit"
    return f"Successfully appended {len(text_to_append)} characters to document {document_id}. Link: {link}"


@server.tool()
@handle_http_errors("delete_doc_range", service_type="docs")
@require_google_service("docs", "docs_write")
async def delete_doc_range(
    ...
    """
    Deletes content within a specified range from a Google Document.

    Args:
        user_google_email: User's Google email address
        document_id: ID of the document to update
        start_index: Start position of content to delete (inclusive, 1-based)
        end_index: End position of content to delete (exclusive)

    Returns:
        str: Confirmation message with operation details
    """
    logger.info(f"[delete_doc_range] Doc={document_id}, range={start_index}-{end_index}")
    ...
    if start_index < 1:
        return "Error: start_index must be at least 1 (cannot delete the first section break)."
...
async def list_doc_tabs(
    ...
    Args:
        user_google_email: User's Google email address
        document_id: ID of the document to inspect
        include_content_summary: Whether to include character count for each tab

    Returns:
        str: Formatted list of tabs with their properties
    """
    logger.info(f"[list_doc_tabs] Doc={document_id}, include_content={include_content_summary}")
    ...
    for index, tab in enumerate(all_tabs):
        ...
        if is_single_tab:
            result += "**Default Tab:**\n"
            result += f"- Tab ID: {tab_props.get('tabId', 'Unknown')}\n"
            result += f"- Title: {tab_props.get('title', '(Untitled)')}\n"
        else:
            prefix = "└─ " if level > 0 else ""
            result += f"{indent}{prefix}**Tab {index + 1}:** \"{tab_props.get('title', 'Untitled Tab')}\"\n"
            result += f"{indent}   - ID: {tab_props.get('tabId', 'Unknown')}\n"
            result += f"{indent}   - Index: {tab_props.get('index', 'N/A')}\n"

            if tab_props.get("parentTabId"):
                result += f"{indent}   - Parent Tab ID: {tab_props.get('parentTabId')}\n"

        # Include content summary if requested
        if include_content_summary:
            document_tab = tab.get("documentTab", {})
            if document_tab:
                text_length = get_tab_text_length(document_tab)
                content_info = f"{text_length:,} characters" if text_length > 0 else "Empty"
