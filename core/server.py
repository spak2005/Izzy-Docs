...
# OpenAI Apps SDK flag (will be set during server configuration)
_openai_apps_enabled = False
...
def enable_openai_apps_sdk():
    """
    Enable OpenAI Apps SDK routes for ChatGPT integration.

    This registers the endpoints required by the OpenAI Apps SDK:
    ...
    - /oauth2/token - OAuth token endpoint
    """
    global _openai_apps_enabled
    if _openai_apps_enabled:
        logger.debug("OpenAI Apps SDK routes already enabled")
        return

    try:
        from openai_apps.routes import register_openai_apps_routes
        register_openai_apps_routes(server)
        _openai_apps_enabled = True
        logger.info("OpenAI Apps SDK routes enabled for ChatGPT integration")
    except ImportError as e:
        logger.warning(f"Could not enable OpenAI Apps SDK routes: {e}")


def is_openai_apps_enabled() -> bool:
    """Check if OpenAI Apps SDK routes are enabled."""
    return _openai_apps_enabled
...
def configure_server_for_http():
    ...
    # Enable OpenAI Apps SDK routes for ChatGPT integration (HTTP transport only)
    if os.getenv("OPENAI_APPS_SDK_ENABLED", "true").lower() == "true":
        enable_openai_apps_sdk()
